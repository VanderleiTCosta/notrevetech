// js/main.js - VERSÃO FINAL V4 - requestAnimationFrame
console.log("JS Version: FINAL_RAF_V4.0 Loaded");
...
window.openModal = function(productId) {
    ...
    if (!productId) {
        console.error("Tentativa de abrir modal sem um ID de produto. Ação ignorada.");
        return;
    }
    const solution = solutionsData.find((s) => s.id === productId);
    if (!solution || !modal) return;

    const featuresHTML = solution.mainFeatures.map((feature) => `<li><i class="fa-solid fa-check"></i>${feature}</li>`).join("");
    const modalHTML = `
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderProductGrid() {
        ...
        solutionsData.forEach((solution) => {
            const cardHTML = `
                <div class="solution-card fade-in-section ${solution.isComingSoon ? "is-coming-soon" : ""}">
                    <i class="solution-icon ${solution.icon}"></i>
                    <h3 class="solution-title">${solution.title}</h3>
                    <p class="solution-description">${solution.description}</p>
                    <button class="card-action-btn" data-id="${solution.id}" ${solution.isComingSoon ? 'disabled' : ''}>
        ...
        // MUDANÇA CRÍTICA: Usamos requestAnimationFrame para garantir que os listeners
        // são adicionados apenas depois de o navegador estar 100% pronto.
    ...
    function initPricingPage() {
        const pricingContainer = document.getElementById("pricing-selector-container");
        if (!pricingContainer) return;
        const pricingGrid = document.getElementById("pricing-grid-container");
        const activeSolutions = solutionsData.filter((s) => !s.isComingSoon && s.pricingTiers && s.pricingTiers.length > 0);
        ...
        pricingContainer.addEventListener("click", (e) => {
            ...
            if (!button) return;
            pricingContainer.querySelectorAll(".product-selector-btn").forEach((btn) => btn.classList.remove("active"));
    ...
    tiers.forEach((tier) => {
      ...
      // MUDANÇA: Adicionado target="_blank" e rel="noopener noreferrer" no link do botão
      container.innerHTML += `<div class="pricing-card ${featuredClass}">${featuredTag}<h3>${
        ...
        tier.description
      }</p><ul>${featuresList}</ul><a href="${tier.ctaLink}" target="_blank" rel="noopener noreferrer" class="btn ${
    ...
    function initDeveloperPage() {
        ...
        const solutionsWithCode = solutionsData.filter((s) => !s.isComingSoon && s.developerPage && s.developerPage.codeExamples.length > 0);
        ...
        selector.addEventListener("click", (e) => {
            const button = e.target.closest(".product-selector-btn");
            if (!button) return;
            selector.querySelectorAll(".product-selector-btn").forEach((btn) => btn.classList.remove("active"));
            button.classList.add("active");
    ...
    function renderCodeExamplesFor(id, container) {
        ...
        if (!solution || !solution.developerPage) {
            container.innerHTML = "<p class='code-placeholder'>Selecione uma API para ver os exemplos de código.</p>";
            return;
        }
        ...
        if (codeExamples && codeExamples.length > 0) {
            const tabs = codeExamples.map((ex, i) => `<button class="tab-link ${i === 0 ? "active" : ""}" data-tab="${id}-${ex.language}">${ex.language}</button>`).join("");
            const contents = codeExamples.map((ex, i) => `<div id="${id}-${ex.language}" class="tab-content ${i === 0 ? "active" : ""}"><pre><code>${ex.code.trim()}</code></pre></div>`).join("");
    ...
    function setupScrollToTop() {
        ...
        if (btn) {
            window.onscroll = () => (btn.style.display = document.body.scrollTop > 100 || document.documentElement.scrollTop > 100 ? "block" : "none");
            btn.addEventListener("click", () => window.scrollTo({ top: 0, behavior: "smooth" }));
        }
    }

    function setupThemeToggle() {
        const btn = document.getElementById("theme-toggle"), body = document.body;
        const applyTheme = (t) => {
            body.className = t;
            localStorage.setItem("theme", t);
        };
        if (btn) btn.addEventListener("click", () => applyTheme(body.classList.contains("dark-theme") ? "light-theme" : "dark-theme"));
